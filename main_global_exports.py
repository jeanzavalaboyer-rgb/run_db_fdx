...
OUTPUT_SHEET = os.getenv("OUTPUT_SHEET", "Global Exports")
...
# WORKER
# =========================
def fetch_exports_task(db_id: int, db_name: str, last_updated: str):
    try:
        ...
        for e in exports:
            rows_local.append({
                ...
                "db_id": db_id,
                "export_id": e.get("id"),
                "export_name": e.get("name"),
                "export_type": e.get("type") or e.get("export_type"),
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [
            executor.submit(fetch_exports_task, db["db_id"], db["db_name"], last_updated)
            for db in databases
        ]
        ...
        for future in as_completed(futures):
            ...
            if not result.get("ok"):
                ...
                print(f"❌ Error exports DB {result.get('db_id')} ({result.get('db_name')})")
                continue

            rows.extend(result.get("rows", []))
            print(f"✅ Exports OK DB {result.get('db_id')} ({result.get('db_name')})")

    df = pd.DataFrame(rows)

    if not df.empty:
        df = df.sort_values(
            by=["db_name", "export_name"],
            na_position="last"
        ).reset_index(drop=True)

    print("🧹 Haciendo refresh completo de la tab Global Exports...")
    ...
    print(f"🔎 Total exports encontrados: {len(df)}")
